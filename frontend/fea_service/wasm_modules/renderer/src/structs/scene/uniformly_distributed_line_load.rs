...
impl UniformlyDistributedLineLoad
{
        ...
        -> Result<(), JsValue>
    {
        let (axes_cap_vertices_coordinates, axis_cap_vertices_colors_values) = 
            monochrome_cone(
                ...
                props.uniformly_distributed_line_load_cap_height, 
                props.uniformly_distributed_line_load_cap_width, 
                props.uniformly_distributed_line_load_cap_base_points_number, 
                color, 
                props.abs_tol,
                props.rel_tol
            )?;
    ...
    {
        UniformlyDistributedLineLoad::add_load_direction_line(color, primitives, reference_point, direction);
        let norm = (props.uniformly_distributed_line_load_line_length - 
            props.uniformly_distributed_line_load_cap_height) / 
            direction.iter().fold(0.0, |acc, x| acc + x * x).sqrt();
        let base_center_point_coordinates = [direction[0] * norm, direction[1] * norm, 
            direction[2] * norm];
        UniformlyDistributedLineLoad::add_load_direction_cap(props, primitives, direction, 
    ...
    fn get_primitives_for_selection(transformed_uid: [u8; 4], point_1_coordinates: [f32; 3], 
        point_2_coordinates: [f32; 3], qx: f32, qy: f32, qz: f32, props: &Props) -> Result<Primitives, JsValue>
    {
        let mut primitives_for_selection = Primitives::create();
        let color_for_selection = transformed_uid.map(|v| v as f32 / 255.0);
        let step = [
            (point_2_coordinates[0] - point_1_coordinates[0]) / 
                (props.number_of_uniformly_distributed_line_load_arrows - 1) as f32,
            (point_2_coordinates[1] - point_1_coordinates[1]) / 
                (props.number_of_uniformly_distributed_line_load_arrows - 1) as f32,
            (point_2_coordinates[2] - point_1_coordinates[2]) / 
                (props.number_of_uniformly_distributed_line_load_arrows - 1) as f32,
        ];

        let optional_qx_direction = 
            if qx > 0.0 { Some([props.uniformly_distributed_line_load_line_length, 0.0, 0.0]) } 
            else if qx < 0.0 { Some([-props.uniformly_distributed_line_load_line_length, 0.0, 0.0]) }
            else { None };
        if let Some(qx_direction) = optional_qx_direction
        {
            for i in 0..props.number_of_uniformly_distributed_line_load_arrows
            {
                ...
                UniformlyDistributedLineLoad::add_force_arrow(&color_for_selection, &mut primitives_for_selection, 
                    &point_coordinates, &qx_direction, props)?;
            }
        }

        let optional_qy_direction = 
            if qy > 0.0 { Some([0.0, props.uniformly_distributed_line_load_line_length, 0.0]) } 
            else if qy < 0.0 { Some([0.0, -props.uniformly_distributed_line_load_line_length, 0.0]) }
            else { None };
        if let Some(qy_direction) = optional_qy_direction
        {
            for i in 0..props.number_of_uniformly_distributed_line_load_arrows
            {
                let point_coordinates = [
                ...
                UniformlyDistributedLineLoad::add_force_arrow(&color_for_selection, &mut primitives_for_selection, 
                    &point_coordinates, &qy_direction, props)?;
            }
        }

        let optional_qz_direction = 
            if qz > 0.0 { Some([0.0, 0.0, props.uniformly_distributed_line_load_line_length]) } 
            else if qz < 0.0 { Some([0.0, 0.0, -props.uniformly_distributed_line_load_line_length]) }
            else { None };
        if let Some(qz_direction) = optional_qz_direction
        {
            for i in 0..props.number_of_uniformly_distributed_line_load_arrows
            {
                let point_coordinates = [
                ...
                UniformlyDistributedLineLoad::add_force_arrow(&color_for_selection, &mut primitives_for_selection, 
                    &point_coordinates, &qz_direction, props)?;
            }
        }
    ...
    pub fn create(
        ...
        -> Result<Self, JsValue>
    {
        let primitives_for_selection = UniformlyDistributedLineLoad::get_primitives_for_selection(
            transformed_uid, point_1_coordinates, point_2_coordinates, qx, qy, qz, props)?;

        let color_visible = props.uniformly_distributed_line_load_color;
        ...
        Ok(UniformlyDistributedLineLoad { transformed_uid, primitives_for_selection, primitives_visible, 
    ...
    pub fn update_point_coordinates(&mut self, point_coordinates: [f32; 6], props: &Props)
    {
        let point_1_coordinates = &point_coordinates[..3];
        let point_2_coordinates = &point_coordinates[3..];
        ...
        for i in 0..props.number_of_uniformly_distributed_line_load_arrows
        {
            let new_point_coordinates = [
                point_1_coordinates[0] + step[0] * i as f32,
                point_1_coordinates[1] + step[1] * i as f32, 
                point_1_coordinates[2] + step[2] * i as f32,
            ];
            for _ in 0..self.primitives_for_selection.get_ref_lines_endpoints_coordinates().len() / 
                (3 * props.number_of_uniformly_distributed_line_load_arrows) as usize
            {
                arrows_lines_reference_points.extend(&new_point_coordinates);
            }
            for _ in 0..self.primitives_for_selection.get_ref_triangles_vertices_coordinates().len() / 
                (3 * props.number_of_uniformly_distributed_line_load_arrows) as usize
            {
                arrows_caps_reference_points.extend(&new_point_coordinates);
            }
        }
...
impl SelectedObjectTrait for UniformlyDistributedLineLoad {}

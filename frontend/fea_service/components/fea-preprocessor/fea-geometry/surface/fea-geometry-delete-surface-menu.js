...
import { DELETE_SURFACE_MESSAGE_HEADER } from "../../../../consts/actions_router_consts.js";
import { CLIENT_MESSAGE_HEADER, GET_WASM_LOADING_STATUS_MESSAGE_HEADER } from "../../../../consts/fea_app_consts.js";


class FeaGeometryDeleteSurfaceMenu extends HTMLElement {
    constructor() {
        super();
        ...
        this.shadowRoot.querySelector(".apply-button").addEventListener("click", () => this.deleteSurface());

        this.shadowRoot.querySelector(".cancel-button").addEventListener("click", () => this.cancelSurfaceDelete());
    ...
    defineSurfaceNumberOptions() {
        const surfaceDeleteNumberSelect = this.shadowRoot.querySelector(".surface-number");
        for (let i = surfaceDeleteNumberSelect.length - 1; i >= 0; i--) {
            surfaceDeleteNumberSelect.options[i] = null;
        }
        if (this.props.store.surfaces_shelf.size > 0) {
            ...
            for (let i = 0; i < surfacesNumbers.length; i++) {
                ...
                deleteOption.value = surfacesNumbers[i];
                deleteOption.innerHTML = surfacesNumbers[i];
                surfaceDeleteNumberSelect.appendChild(deleteOption);
            }
        }
    }
    ...
    deleteSurface() {
        ...
        const message = { [DELETE_SURFACE_MESSAGE_HEADER]: {
...
export default FeaGeometryDeleteSurfaceMenu;

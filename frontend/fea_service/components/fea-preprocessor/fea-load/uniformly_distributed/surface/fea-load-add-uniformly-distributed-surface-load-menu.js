...
import { ADD_UNIFORMLY_DISTRIBUTED_SURFACE_LOAD_MESSAGE_HEADER } from "../../../../../consts/actions_router_consts.js";
import { CLIENT_MESSAGE_HEADER, GET_WASM_LOADING_STATUS_MESSAGE_HEADER } from "../../../../../consts/fea_app_consts.js";


class FeaLoadAddUniformlyDistributedSurfaceLoadMenu extends HTMLElement {
    constructor() {
        super();
        ...
        this.shadowRoot.querySelector(".apply-button").addEventListener("click", () => this.addDistributedSurfaceLoad());

        this.shadowRoot.querySelector(".cancel-button").addEventListener("click", 
            () => this.cancelDistributedSurfaceLoadAddition());
    ...
    defineDistributedSurfaceLoadOptions() {
        ...
        if (this.props.store.surfaces_shelf.size > 0) {
            ...
            for (let i = 0; i < surfacesNumbers.length; i++) {
                ...
                surfaceNumberOption.value = surfacesNumbers[i];
                surfaceNumberOption.innerHTML = surfacesNumbers[i];
                surfaceNumberSelect.appendChild(surfaceNumberOption);
            }

            if (this.props.store.surfaces_shelf.get(
                surfacesNumbers[0]).optional_uniformly_distributed_surface_load) {
                this.shadowRoot.querySelector(".px").value = 
                    this.props.store.surfaces_shelf.get(surfacesNumbers[0]).optional_uniformly_distributed_surface_load[1];
                this.shadowRoot.querySelector(".py").value = 
                    this.props.store.surfaces_shelf.get(surfacesNumbers[0]).optional_uniformly_distributed_surface_load[2];
                this.shadowRoot.querySelector(".pz").value = 
                    this.props.store.surfaces_shelf.get(surfacesNumbers[0]).optional_uniformly_distributed_surface_load[3];
            } else {
                this.shadowRoot.querySelector(".px").value = "";
                this.shadowRoot.querySelector(".py").value = "";
                this.shadowRoot.querySelector(".pz").value = "";
    ...
    updateDistributedSurfaceLoadValues() {
        ...
        if (this.props.store.surfaces_shelf.get(
            parseInt(selectedSurfaceNumber)).optional_uniformly_distributed_surface_load) {
            this.shadowRoot.querySelector(".px").value = this.props.store.surfaces_shelf.get(
                parseInt(selectedSurfaceNumber)).optional_uniformly_distributed_surface_load[1];
            this.dropHighlight(this.shadowRoot.querySelector(".px"));
            this.shadowRoot.querySelector(".py").value = this.props.store.surfaces_shelf.get(
                parseInt(selectedSurfaceNumber)).optional_uniformly_distributed_surface_load[2];
            this.dropHighlight(this.shadowRoot.querySelector(".py"));
            this.shadowRoot.querySelector(".pz").value = this.props.store.surfaces_shelf.get(
                parseInt(selectedSurfaceNumber)).optional_uniformly_distributed_surface_load[3];
    ...
    addDistributedSurfaceLoad() {
        ...
        const message = { [ADD_UNIFORMLY_DISTRIBUTED_SURFACE_LOAD_MESSAGE_HEADER]: {
            action_id: this.props.actionId,
            surface_number: selectedSurfaceNumberField.value, 
            px: inputtedPXField.value, py: inputtedPYField.value, pz: inputtedPZField.value,
        }};
    ...
    cancelDistributedSurfaceLoadAddition() {
        if (this.props.store.surfaces_shelf.size > 0) {
            this.defineDistributedSurfaceLoadOptions();
        }
        this.shadowRoot.querySelector(".surface-number-filter").value = null;
...
export default FeaLoadAddUniformlyDistributedSurfaceLoadMenu;

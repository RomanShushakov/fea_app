...
import { DELETE_BEAM_SECTION_MESSAGE_HEADER } from "../../../../consts/actions_router_consts.js";
...
class FeaSectionDeleteBeamMenu extends HTMLElement {
    ...
    defineBeamSectionNameOptions() {
        ...
        for (let i = 0; i < this.props.store.beam_sections_shelf.length; i++) {
            ...
            deleteOption.value = this.props.store.beam_sections_shelf[i].name.replace(/['"]+/g, "");
            deleteOption.innerHTML = this.props.store.beam_sections_shelf[i].name.replace(/['"]+/g, "");
    ...
    deleteBeamSection() {
        ...
        const deletedBeamSectionData = this.props.store.beam_sections_shelf
            .find(beamSection => beamSection.name == `"${selectedBeamSectionNameField.value}"`);
        
        const message = { [DELETE_BEAM_SECTION_MESSAGE_HEADER]: { 
            action_id: this.props.actionId, name: deletedBeamSectionData.name.replace(/['"]+/g, "") 
        }};

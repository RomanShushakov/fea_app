...
import { DELETE_PLATE_SECTION_MESSAGE_HEADER } from "../../../../consts/actions_router_consts.js";
import { CLIENT_MESSAGE_HEADER, GET_WASM_LOADING_STATUS_MESSAGE_HEADER } from "../../../../consts/fea_app_consts.js";


class FeaSectionDeletePlateMenu extends HTMLElement {
    constructor() {
        super();
        ...
        this.shadowRoot.querySelector(".apply-button").addEventListener("click", () => this.deletePlateSection());

        this.shadowRoot.querySelector(".cancel-button").addEventListener("click", () => this.cancelPlateSectionDelete());
    ...
    definePlateSectionNameOptions() {
        const plateSectionDeleteNameSelect = this.shadowRoot.querySelector(".plate-section-name");
        for (let i = plateSectionDeleteNameSelect.length - 1; i >= 0; i--) {
            plateSectionDeleteNameSelect.options[i] = null;
        }
        for (let i = 0; i < this.props.store.plate_sections_shelf.length; i++) {
            let deleteOption = document.createElement("option");
            deleteOption.value = this.props.store.plate_sections_shelf[i].name.replace(/['"]+/g, "");
            deleteOption.innerHTML = this.props.store.plate_sections_shelf[i].name.replace(/['"]+/g, "");
            plateSectionDeleteNameSelect.appendChild(deleteOption);
        }
    }
    ...
    deletePlateSection() {
        ...
        const deletedPlateSectionData = this.props.store.plate_sections_shelf
            .find(plateSection => plateSection.name == `"${selectedPlateSectionNameField.value}"`);
        
        const message = { [DELETE_PLATE_SECTION_MESSAGE_HEADER]: { 
            action_id: this.props.actionId, name: deletedPlateSectionData.name.replace(/['"]+/g, "") 
        }};
    ...
    cancelPlateSectionDelete() {
        if (this.props.store.plate_sections_shelf.length > 0) {
            this.definePlateSectionNameOptions();
        }
        this.shadowRoot.querySelector(".plate-section-name-filter").value = null;
        const selectedPlateSectionNameForDeleteField = this.shadowRoot.querySelector(".plate-section-name");
        this.dropHighlight(selectedPlateSectionNameForDeleteField);
...
export default FeaSectionDeletePlateMenu;

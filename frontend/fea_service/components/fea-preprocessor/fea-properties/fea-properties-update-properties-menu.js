...
import { UPDATE_PROPERTIES_MESSAGE_HEADER } from "../../../consts/actions_router_consts.js";
...
class FeaPropertiesUpdatePropertiesMenu extends HTMLElement {
    ...
    updateSelectedPropertiesData(selectedPropertiesName) {
        const selectedPropertiesInProps = this.props.store.properties_shelf
    ...
    definePropertiesNameOptions() {
        ...
        if (this.props.store.properties_shelf.length > 0) {
            for (let i = 0; i < this.props.store.properties_shelf.length; i++) {
                ...
                updateOption.value = this.props.store.properties_shelf[i].name.replace(/['"]+/g, "");
                updateOption.innerHTML = this.props.store.properties_shelf[i].name.replace(/['"]+/g, "");
                propertiesUpdateNameSelect.appendChild(updateOption);
            }
            const selectedSectionType = this.props.store.properties_shelf[0].cross_section_type;
            const selectedMaterialName = this.props.store.properties_shelf[0].material_name;
            const selectedSectionName = this.props.store.properties_shelf[0].cross_section_name;
            const crossSectionTypeSelect = this.shadowRoot.querySelector(".cross-section-type");
    ...
    updateProperties() {
        ...
        const oldPropertiesValues = this.props.store.properties_shelf.find(

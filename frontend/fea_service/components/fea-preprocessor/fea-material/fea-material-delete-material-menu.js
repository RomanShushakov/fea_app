...
import { DELETE_MATERIAL_MESSAGE_HEADER } from "../../../consts/actions_router_consts.js";
import { 
...
class FeaMaterialDeleteMaterialMenu extends HTMLElement {
    ...
    defineMaterialNameOptions() {
        ...
        for (let i = 0; i < this.props.store.materials_shelf.length; i++) {
            ...
            deleteOption.value = this.props.store.materials_shelf[i].name.replace(/['"]+/g, "");
            deleteOption.innerHTML = this.props.store.materials_shelf[i].name.replace(/['"]+/g, "");
    ...
    deleteMaterial() {
        ...
        const deletedMaterialData = this.props.store.materials_shelf.find(
            material => material.name == `"${selectedMaterialNameField.value}"`);

        const message = { [DELETE_MATERIAL_MESSAGE_HEADER]: { 
            action_id: this.props.actionId, name: deletedMaterialData.name.replace(/['"]+/g, "") 
        }};
